...
public class exercise_03b_dilation {
	...
	public static void main(String[] args) {
		...
		if (args.length == 3) {
			...
			try {
				exercise_03b_dilation.process(filein1, size, fileout);
			} catch (IOException e) {
	...
	public static void process(String filein1, int size, String fileout) throws IOException {
		 ...
		 */
		//GrayU8 img_out = getDilationPic(size, img_filein1);
		GrayU8 img_out = dilation(size, img_filein1);

		UtilImageIO.saveImage(img_out, fileout);

	}

	public static GrayU8 dilation(int size, GrayU8 img_filein1)
	{
		//Adapt size
		size=2*size+1;
        ...
        for(int y = 0; y < height; y++){
            for(int x = 0; x < width; x++){
                ...
                //save highest value
                output[x+y*height] = buff[(size*size) - 1];
            }
        }
